...
RSpec.describe PgOnlineSchemaChange::Query do
  ...
  describe ".alter_statement_for" do
    it "returns index statements for shadow table" do
      client = PgOnlineSchemaChange::Client.new(client_options)
      ...
      result = described_class.get_indexes_for(client, "new_books")
      expect(result).to eq([
                             "CREATE UNIQUE INDEX books_pkey ON public.new_books USING btree (user_id)",
                             "CREATE UNIQUE INDEX books_username_key ON public.new_books USING btree (username)",
                             "CREATE UNIQUE INDEX books_email_key ON public.new_books USING btree (email)"
                           ])
    end
  end
end
